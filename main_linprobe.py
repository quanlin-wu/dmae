...
from util.datasets import DatasetWithInterval
...
def get_args_parser():
    parser = argparse.ArgumentParser('DMAE linear probing for image classification', add_help=False)
...
def main(args):
    ...
    print(dataset_train)
    print(dataset_val)
    dataset_certify = DatasetWithInterval(dataset_val, args.sample_interval)
    ...
    if args.con_reg:
        print("enable consistency regularization with lbd %.1f and eta %.1f" % (args.reg_lbd, args.reg_eta))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % 1 == 0:
            test_stats_r0 = evaluate_radius_0(data_loader_certify, model, device, args.sigma, stride=10)
