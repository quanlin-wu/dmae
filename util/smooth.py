...
transform = transforms.Resize((224, 224), interpolation=PIL.Image.BICUBIC)
...
class Smooth(object):
    ...
    def certify(self, x: torch.tensor, n0: int, n: int, alpha: float, batch_size: int, target: int) -> (int, float):
    ...
    def _sample_noise(self, x: torch.tensor, num: int, batch_size) -> torch.tensor:
        ...
        with torch.no_grad():
            ...
            for _ in range(ceil(num / batch_size)):
                ...
                index, count = predictions.unique(return_counts=True)
                counts[index] += count
            return counts
